...
def cross_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    '''
    print("cross_view_gallery_evaluation")
...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    print("single_view_gallery_evaluation")
    ...
    # print("probe_seq_dict[dataset].items()", probe_seq_dict[dataset].items(), len(probe_seq_dict[dataset].items()))
    # print("view_list", view_list, len(view_list))
    # print("label", label, len(label))
    
    nan_file = open("nan.txt", "w")
     
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        acc[type_] = np.zeros((view_num, view_num)) - 1.
        for (v1, probe_view) in tqdm(enumerate(view_list)):
            pseq_mask = np.isin(seq_type, probe_seq) & np.isin(
                view, probe_view)
            probe_x = feature[pseq_mask, :]
            probe_y = label[pseq_mask]
            ...
            for (v2, gallery_view) in enumerate(view_list):
                ...
                path_dist = "./matrices/temp/"
                cpu_dist = dist.cpu().numpy()
                new_frame = pd.DataFrame(cpu_dist, columns=gallery_y, index=probe_y)
                new_frame.to_csv(os.path.join(path_dist, f"gallery{gallery_view}_probe{probe_view}.csv"))
